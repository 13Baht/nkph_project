...
$s_status = "1";
if ($st == "edit") {
	$sql = "select * from strategic_hospital where strategic_hospital_id = \"$strategic_hospital_id\"";
	$result = mysql_query($sql);
	$r = mysql_fetch_array($result);
	$s_status = $r['strategic_hospital_status'];
}
?>
...
<title>จัดการประเด็นยุทธศาสตร์โรงพยาบาล</title>
...
function saveData(st) {
	...
	var strategic_hospital_name = frm.strategic_hospital_name.value;
	var s_status = "1";
	if (frm.s_status[1].checked == true) {
		s_status = "0";
	}
	var strategic_hospital_id = frm.strategic_hospital_id.value;
	if (year_budget_id == "") {
		alert('กรุณาเลือกปีงบประมาณ');
		frm.year_budget_id.focus();
		return false;
	}
	if (strategic_hospital_name == "") {
		alert('กรุณาระบุชื่อประเด็นยุทธศาสตร์โรงพยาบาล');
		frm.strategic_hospital_name.focus();
		return false;
	}
	var url = "strategic_hospital_save.php?year_budget_id=" + year_budget_id + "&strategic_hospital_name=" + encodeURIComponent(strategic_hospital_name) + "&strategic_hospital_id=" + strategic_hospital_id + "&s_status=" + s_status + "&st=" + st;
	LoadData(url, "tSave");
...
<table width="500" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" colspan="2" align="center" bgcolor="#E9E9E9" class="textBlackBold" style="border-bottom:1px solid #CCC; border-left:1px solid #CCC; border-top:1px solid #CCC; border-right:1px solid #CCC;">จัดการประเด็นยุทธศาสตร์โรงพยาบาล</td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="2">
      <tr>
        ...
        <td width="73%" height="25" align="left"><select name="year_budget_id" id="year_budget_id" style="width:90px;">
						...
						while ($r_budget = mysql_fetch_array($result_budget)) {
							if ($r['strategic_hospital_year'] == $r_budget['year_budget_id']) {
      ...
      <tr>
        ...
        <td height="25" align="left"><input name="strategic_hospital_name" type="text" id="strategic_hospital_name" style="width:300px;" value="<?php echo $r['strategic_hospital_name']; ?>" maxlength="250" /></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td height="25" align="left"><input type="button" name="button" id="button" value="  บันทึกข้อมูล  " onclick="saveData('<?php echo $st; ?>')" />
          ...
          <input name="strategic_hospital_id" type="hidden" id="strategic_hospital_id" value="<?php echo $strategic_hospital_id; ?>" /></td>
      </tr>
      <tr>
        <td height="25" align="left">&nbsp;</td>
