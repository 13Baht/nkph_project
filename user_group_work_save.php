...
<body>
<?php
$person_id = $_REQUEST['group_work_id'];
$st = $_REQUEST['st'];
include "config.php";
if ($st == "save") {
	//ตรวจสอบว่ามีข้อมูลแล้วหรือไม่
	$sql1 = "select person_id from user_group_work where person_id = \"$person_id\"";
...
if ($st == "edit" || $st == "del") {
	//ลบข้อมูลออกก่อน
	$sql_d = "delete from user_group_work where person_id = \"$person_id\"";
	$result_d = mysql_query($sql_d);
	//
	$sql_d2 = "delete from user_group_work_detail where person_id = \"$person_id\"";
	$result = mysql_query($sql_d2);
}
if ($st == "save" || $st == "edit") {
	//บันทึกข้อมูลใหม่
	$sql = "insert into user_group_work(person_id) values(\"$person_id\")";
	...
	while ($r_sd = mysql_fetch_array($result_sd)) {
		...
		if ($ck == "1") {
			...
			$sql = "insert into user_group_work_detail(person_id, sub_dept_id) values('$person_id', '$r_sd[sub_dept_id]')";
			$result = mysql_query($sql);
		}
	} //end while
}
