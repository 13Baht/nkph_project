        ...
        <td><table width="960" border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td width="719" valign="top"><table width="719" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="7" align="left"><img src="images/top-left.jpg" width="7" height="45" /></td>
                <td width="707" align="left" background="images/top2.jpg" class="textWhiteBold"><!-- InstanceBeginEditable name="EditRegion3" -->จัดการข้อมูลหัวหน้ากลุ่มงาน<!-- InstanceEndEditable --></td>
                <td width="7" align="right"><img src="images/top-right.jpg" width="8" height="45" /></td>
              </tr>
              <tr>
                <td background="images/left.jpg">&nbsp;</td>
...
function chkData() {
	...
	if (frm.group_work_id.value == "") {
		alert('กรุณาเลือกบุคลากรก่อนครับ');
		return false;
	}
...
function popupsearchGroupWork(module, output1, output2) {
	var str = Math.random();
	w = screen.availWidth;
	h = screen.availHeight;
	var popW = 550, popH = 400;
	var leftPos = (w-popW)/2, topPos = (h-popH)/2;
	window.open("popup_person.php?module=" + module + "&output1=" + output1 + "&output2=" + output2 + "&str=" + str, "loadorder", "width=" + popW + ", height=" + popH + ", top=" + topPos + ", left=" + leftPos + ", menubar=no, resizeable=no, toolbar=no, scrollbars=yes");
}
</script>
                <form method="post" action="user_group_work_save.php" name="frm1" onsubmit="return chkData()">
                  <table width="670" border="0" cellspacing="0" cellpadding="0">
                    <?php if ($st == "save") { ?>
                    <tr>
                      <td width="99" height="25" align="left" class="textBlackNormal">ชื่อ - สกุล :</td>
                      <td width="571" height="25" align="left">
                      <input name="group_work_name" type="text" id="group_work_name" readonly="readonly" style="width:200px;" />
                      <a href="javascript:;" onClick="popupsearchGroupWork('frm1', 'document.frm1.group_work_name', 'document.frm1.group_work_id')" title="คลิกเพื่อค้นหารายชื่อ"><img src="images/detail.png" width="23" height="23" border="0" align="absmiddle" /></a>
                      </td>
                    </tr>
                    <?php } //end save ?>
                    <?php if ($st == "edit") { ?>
                    <tr>
                      <td width="99" height="25" class="textBlackNormal">ชื่อ - สกุล :</td>
                      <td width="571" height="25" align="left" class="textGreenNormal"><?php
                      include "config_ksh.php";
					  $sql_p = "select p2.prefix_name, p.person_firstname, p.person_lastname from personal p inner join prefix p2 on p.person_prefix = p2.prefix_id where p.person_id = \"$person_id\"";
					  $result_p = mysql_query($sql_p);
					  $r_p = mysql_fetch_array($result_p);
					  $full_name = $r_p['prefix_name'].$r_p['person_firstname']."&nbsp;".$r_p['person_lastname'];
					  echo $full_name;
					  ?></td>
                    </tr>
                    <?php } //end edit ?>
			...
			function check_uncheck_all(checkname, field) {
				for (i = 0; i < checkname.length; i++) {
                    ...
                    <tr>
                      <td height="25" class="textBlackNormal">หน่วยงานที่ดูแล :</td>
                      <td height="25" align="left" class="textBlackNormal"><label>
              <input type="checkbox" name="chk_all" id="chk_all" onclick="check_uncheck_all(document.frm1.sd, this)" />
              เลือกหน่วยงานทั้งหมด</label></td>
                    </tr>
                    <tr>
			  ...
			  $result_sd = mysql_db_query($db, $sql_sd);
			  $i = 1;
			  while ($r_sd = mysql_fetch_array($result_sd)) {
				  if ($st == "edit") {
					  $sqlng = "select sub_dept_id from user_group_work_detail where person_id = '$person_id' and sub_dept_id = '$r_sd[sub_dept_id]'";
					  $resultng = mysql_db_query($db, $sqlng);
					  $rng = mysql_fetch_row($resultng);
				  }
				  if ($i == 4) {
					$i = 1;
				  }
			  ...
			  ?>
                        <td width="220" height="25" align="left" class="textBlackNormal"><?php
                  echo "<label><input type='checkbox' name='sd_$r_sd[sub_dept_id]' id='sd' value='1'";
				  if ($st == "edit") {
					  if ($rng[0] != "") {
						  echo " checked='checked'";
					  }
				  }
				  echo ">$r_sd[sub_dept_name]</label>";
				  ?></td>
                          <?php
				if ($i == 3) {
					echo "</tr>";
                ...
                $i++;
                    ...
                    <tr>
                      ...
                      <td height="25" align="left"><input type="submit" name="button2" id="button2" value="  บันทึก  " />
                        <input type="hidden" name="st" id="st" value="<?php echo $st; ?>" /><input name="group_work_id" type="hidden" id="group_work_id" value="<?php echo $person_id; ?>" /></td>
                    </tr>
                  </table>
